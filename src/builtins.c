...
#define MAKE_BUILTIN(fn)                                                       \
  ((struct object){.type = OBJ_BUILTIN, .value = {.fn_builtin = &fn}})
...
// here we store the built-in function directly on the pointer by casting it to
// the wrong value this saves us a level of indirection when calling built-in
// functions
const struct {
  const char *name;
...
inline struct object get_builtin_by_index(const uint8_t index) {
  return builtin_functions[index].fn_obj;
}

struct object get_builtin(const char *name) {
  for (int i = 0; i < sizeof(builtin_functions) / sizeof(builtin_functions[0]);
       i++) {
...
static struct object builtin_len(const struct object_list *args) {
  if (args->size != 1) {
    return make_error_object("wrong number of arguments: expected 1, got %d",
                             args->size);
  }
  ...
  default:
    return make_error_object("argument to len() not supported: got %s",
                             object_type_to_str(arg.type));
    break;
  }
}
...
static struct object builtin_array_pop(const struct object_list *args) {
  ...
  if (args->values[0].type != OBJ_ARRAY) {
    return make_error_object("invalid argument: expected ARRAY, got %s",
