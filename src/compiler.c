...
static int compile_statement(struct compiler *compiler,
                             const struct statement *statement);
static int compile_expression(struct compiler *compiler,
...
struct compiler *compiler_new() {
  ...
  scope.instructions->bytes =
      calloc(scope.instructions->cap, sizeof *scope.instructions->bytes);
...
struct compiler *compiler_new_with_state(struct symbol_table *t,
...
/* TODO: We probably want dynamic error messages that includes parts of the
 * program string */
...
static uint32_t add_constant(struct compiler *c, struct object obj) {
  c->constants->values[c->constants->size++] = obj;
  return c->constants->size - 1;
}

static void compiler_set_last_instruction(struct compiler *c,
                                          enum opcode opcode, uint32_t pos) {
  struct emitted_instruction previous =
...
static void compiler_remove_last_instruction(struct compiler *c) {
  ...
  c->scopes[c->scope_index].instructions->size =
      c->scopes[c->scope_index].last_instruction.position;
  c->scopes[c->scope_index].last_instruction =
      c->scopes[c->scope_index].previous_instruction;
}

static void compiler_replace_instruction(struct compiler *c, uint32_t pos,
...
static void compiler_replace_last_instruction(struct compiler *c,
                                              struct instruction *ins) {
...
static bool compiler_last_instruction_is(struct compiler *c,
                                         enum opcode opcode) {
...
static void compiler_change_operand(struct compiler *c, uint32_t pos,
                                    uint16_t operand) {
  enum opcode opcode = c->scopes[c->scope_index].instructions->bytes[pos];
  for (int8_t byte_idx = lookup(opcode).operand_widths[0] - 1, i = 1;
       byte_idx >= 0; byte_idx--, i++) {
    c->scopes[c->scope_index].instructions->bytes[pos + i] =
        (uint8_t)(operand >> (byte_idx * 8));
  }
}

static uint32_t compiler_emit_va(struct compiler *c, enum opcode opcode,
                                 va_list operands) {
...
static int compile_block_statement(struct compiler *compiler,
                                   const struct block_statement *block) {
  int err;
...
static int compile_statement(struct compiler *c, const struct statement *stmt) {
  int err;
  switch (stmt->type) {
  case STMT_EXPR: {
  ...
  case STMT_LET: {
    ...
    compiler_emit(
        c, s->scope == SCOPE_GLOBAL ? OPCODE_SET_GLOBAL : OPCODE_SET_LOCAL,
        s->index);
  } break;
...
void compiler_change_jump_placeholders(struct compiler *c, uint32_t pos_start,
                                       uint32_t pos_end,
                                       int32_t placeholder_value,
                                       int32_t actual_value) {
  for (uint32_t p = pos_start; p < pos_end;) {
    enum opcode op = c->scopes[c->scope_index].instructions->bytes[p];
    if (op == OPCODE_JUMP &&
        read_uint16(&c->scopes[c->scope_index].instructions->bytes[p + 1]) ==
            placeholder_value) {
...
static int compile_expression(struct compiler *c,
  ...
  case EXPR_POSTFIX: {
    ...
    struct symbol *s =
        symbol_table_resolve(c->symbol_table, expr->postfix.left->ident.value);
    if (s == NULL) {
  ...
  case EXPR_IF: {
    ...
    uint32_t jump_if_not_true_pos =
        compiler_emit(c, OPCODE_JUMP_NOT_TRUE, 9999);
    ...
    /* same story here, replace placeholder position with actual jump to
     * position */
    uint32_t after_alternative_pos =
        c->scopes[c->scope_index].instructions->size;
  ...
  case EXPR_FUNCTION: {
    ...
    for (uint32_t i = 0; i < expr->function.parameters.size; i++) {
      symbol_table_define(c->symbol_table,
                          expr->function.parameters.values[i].value);
    }
    ...
    if (compiler_last_instruction_is(c, OPCODE_POP)) {
      compiler_replace_last_instruction(c,
  ...
  case EXPR_WHILE: {
    ...
    uint32_t after_conseq_pos = c->scopes[c->scope_index].instructions->size;
    compiler_change_operand(c, jump_if_not_true_pos, after_conseq_pos);
    compiler_change_jump_placeholders(c, loop_start_pos, loop_end_pos,
                                      JUMP_PLACEHOLDER_BREAK, after_conseq_pos);
    compiler_change_jump_placeholders(c, loop_start_pos, loop_end_pos,
                                      JUMP_PLACEHOLDER_CONTINUE, before_pos);
  } break;
  ...
  case EXPR_ASSIGN: {
    if (expr->assign.left->type == EXPR_IDENT) {
      struct symbol *s =
          symbol_table_resolve(c->symbol_table, expr->assign.left->ident.value);
      if (s == NULL) {
        return COMPILE_ERR_UNKNOWN_IDENT;
      }
