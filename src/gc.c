...
void gc_add(struct vm *restrict vm, struct object obj) {
...
void gc(struct vm *restrict vm) {
// we want to run the garbage collector pretty much all the time when in debug
// mode so this code gets properly exercised
#ifndef TEST_MODE
  ...
  // traverse VM constants, stack and globals and mark every object that is
  // reachable
  ...
  for (uint32_t i = 0; i < GLOBALS_SIZE && vm->globals[i].type != OBJ_NULL;
       i++) {
