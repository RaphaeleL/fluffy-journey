...
const char *object_type_to_str(enum object_type t) {
  static const char *object_names[] = {
      "NULL",  "BOOLEAN", "INTEGER", "BUILTIN",
      "ERROR", "STRING",  "ARRAY",   "COMPILED_FUNCTION",
  };
...
struct object make_boolean_object(bool value) {
  return (struct object){.type = OBJ_BOOL, .value.boolean = value};
}
...
struct object concat_string_objects(struct string *left, struct string *right) {
  struct object obj =
      make_string_object_with_length(left->value, left->length + right->length);
...
struct object make_compiled_function_object(const struct instruction *ins,
  ...
  struct compiled_function *f =
      malloc(sizeof(struct compiled_function) + ins->size);
  assert(f != NULL);
...
void append_to_object_list(struct object_list *list, struct object obj) {
  if (list->size == list->cap) {
    ...
    list->values = (struct object *)realloc(list->values,
                                            list->cap * sizeof(struct object));
    assert(list->values != NULL);
  }
...
void object_to_str(char *str, struct object obj) {
  ...
  case OBJ_INT:
    sprintf(tmp, "%lld", obj.value.integer);
    strcat(str, tmp);
    break;
