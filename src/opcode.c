...
inline const struct definition lookup(enum opcode opcode) {
...
struct instruction *flatten_instructions_array(struct instruction *arr[],
                                               const uint32_t size) {
...
uint32_t read_operands(uint32_t dest[], struct definition def,
                       const struct instruction *ins, uint32_t offset) {
