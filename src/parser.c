...
static enum precedence get_token_precedence(const struct token t) {
  switch (t.type) {
  default:
    return LOWEST;
    break;
...
static void add_parsing_error(struct parser *p, const char *_format, ...) {
  va_list args;

  // create format string with filename, line # & line pos prepended
  char format[256];
  sprintf(format, "%d:%d: %s", p->current_token.line, p->current_token.pos,
          _format);

  // alocate space for error message
  p->error_messages =
      realloc(p->error_messages, (p->errors + 1) * sizeof(char *));
  assert(p->error_messages != NULL);
...
static bool current_token_is(struct parser *p, enum token_type t) {
  return t == p->current_token.type;
}

static bool next_token_is(struct parser *p, enum token_type t) {
  return t == p->next_token.type;
}

static void skip_forward(struct parser *p) {
  // skip forward to next semicolon
  while (!current_token_is(p, TOKEN_SEMICOLON) &&
         !current_token_is(p, TOKEN_EOF)) {
    next_token(p);
  }
}

static bool advance_to_next_token(struct parser *p, enum token_type t) {
  if (next_token_is(p, t)) {
    next_token(p);
    return true;
  }

  add_parsing_error(p, "Expected \"%s\", got \"%s\" instead",
                    token_type_to_str(t), p->next_token.literal);
  skip_forward(p);

  return false;
}

static struct expression *make_expression(enum expression_type type,
                                          struct token tok) {
...
static bool parse_let_statement(struct parser *p, struct statement *stmt) {
  stmt->type = STMT_LET;
...
static struct expression *parse_identifier_expression(const struct parser *p) {
...
static struct expression *parse_string_literal(struct parser *p) {
...
static struct expression_list
parse_expression_list(struct parser *p, const enum token_type end_token) {
...
static struct expression *parse_array_literal(struct parser *p) {
  struct expression *expr = make_expression(EXPR_ARRAY, p->current_token);
  expr->array = parse_expression_list(p, TOKEN_RBRACKET);
  return expr;
}

static struct expression *parse_slice_expression(struct parser *p,
...
static struct expression *parse_index_expression(struct parser *p,
...
static struct expression *parse_call_expression(struct parser *p,
                                                struct expression *left) {
...
static struct expression *parse_assignment_expression(struct parser *p,
                                                      struct expression *left) {
  if (left->type != EXPR_IDENT && left->type != EXPR_INDEX) {
    add_parsing_error(
        p,
        "Invalid assignment left-hand side. Expected IDENT or INDEX, got %s.",
        expression_type_to_str(left->type));
    return NULL;
  }
...
static struct expression *parse_postfix_expression(struct parser *p,
                                                   struct expression *left) {
...
static struct expression *parse_infix_expression(struct parser *p,
                                                 struct expression *left) {
...
static struct expression *parse_boolean_expression(struct parser *p) {
  struct expression *expr = make_expression(EXPR_BOOL, p->current_token);
  expr->boolean = current_token_is(p, TOKEN_TRUE);
  return expr;
}

static struct expression *parse_grouped_expression(struct parser *p) {
  next_token(p);
...
static struct block_statement *create_block_statement(uint32_t cap) {
...
static void add_statement_to_block(struct block_statement *b,
                                   struct statement *s) {
  if (b->size + 1 == b->cap) {
    b->cap *= 2;
...
static struct block_statement *parse_block_statement(struct parser *p) {
  struct block_statement *b = create_block_statement(4);
  next_token(p); // LBRACE

  while (!current_token_is(p, TOKEN_RBRACE) &&
         !current_token_is(p, TOKEN_EOF)) {
    struct statement s;
...
static struct expression *parse_for_expression(struct parser *p) {
...
static struct expression *parse_while_expression(struct parser *p) {
...
static struct expression *parse_if_expression(struct parser *p) {
...
static struct identifier_list parse_function_parameters(struct parser *p) {
  ...
  while (next_token_is(p, TOKEN_COMMA)) {
    ...
    if (params.size >= params.cap) {
      params.cap *= 2;
      params.values =
          realloc(params.values, params.cap * sizeof *params.values);
    }
  }
...
static struct expression *parse_function_literal(struct parser *p) {
...
static struct expression *parse_expression(struct parser *p,
  ...
  case TOKEN_SEMICOLON:
    ...
    break;
  default:
    add_parsing_error(p, "Expected expression, got \"%s\".",
  ...
  while (!next_token_is(p, TOKEN_SEMICOLON) &&
         precedence < get_token_precedence(p->next_token)) {
    ...
    case TOKEN_PERCENT:
      ...
      // Test: foo++
      // Test: foo--
      if (left->type == EXPR_IDENT && next_token_is(p, type) &&
          (next_token_is(p, TOKEN_PLUS) || next_token_is(p, TOKEN_MINUS))) {
...
static int parse_loop_control_statement(struct parser *p, struct statement *s) {
...
struct program *parse_program(struct parser *parser) {
  const int cap = 4;
  struct program *program = (struct program *)malloc(
      sizeof *program + cap * sizeof *program->statements);
  if (!program) {
    err(EXIT_FAILURE, "OUT OF MEMORY");
  }
  ...
  while (parser->current_token.type != TOKEN_EOF) {
    ...
    if (program->size == program->cap) {
      program->cap *= 2;
      program = realloc(program, sizeof(struct program) +
                                     (sizeof(struct statement) * program->cap));
      assert(program != NULL);
      program->statements = (struct statement *)(program + 1);
    }
...
void statement_to_str(char *str, const struct statement *stmt) {
  switch (stmt->type) {
  case STMT_LET:
    let_statement_to_str(str, stmt);
    break;
  case STMT_RETURN:
    return_statement_to_str(str, stmt);
    break;
  case STMT_EXPR:
    expression_to_str(str, stmt->value);
    break;
...
void identifier_list_to_str(char *str,
                            const struct identifier_list *identifiers) {
...
const char *expression_type_to_str(enum expression_type t) {
  const char *names[] = {
      "INFIX", "PREFIX",   "POSTFIX", "INT",    "IDENT", "BOOL",
      "IF",    "FUNCTION", "CALL",    "STRING", "ARRAY", "INDEX",
      "SLICE", "FOR",      "WHILE",   "ASSIGN",
  };
  return names[t];
}
