...
static void print_debug_info(struct vm *vm) {
  char str[BUFSIZ] = {'\0'};
  ...
  printf("\n\nFrame: %2d | IP: %3d/%d | opcode: %12s | operand: ",
         vm->frame_index, ip_now, ip_end, opcode_to_str(*frame->ip));
  ...
  for (int i = 0; i < vm->nconstants; i++) {
    ...
    printf("  %3d: %s = %s\n", i, object_type_to_str(vm->constants[i].type),
           str);
  }
...
struct vm *vm_new_with_globals(struct bytecode *bc,
...
static void
vm_do_binary_integer_operation(struct vm *restrict vm, const enum opcode opcode,
                               struct object *restrict left,
                               const struct object *restrict right) {
  switch (opcode) {
  case OPCODE_ADD:
  ...
  case OPCODE_MODULO:
    ...
    break;
  default:
    err(VM_ERR_INVALID_OPERATOR, "Invalid operator %s for integer operation.",
        opcode_to_str(opcode));
    break;
  }
}

static void vm_do_binary_string_operation(struct vm *restrict vm,
                                          enum opcode opcode,
                                          struct object *restrict left,
                                          const struct object *restrict right) {
  switch (opcode) {
  case OPCODE_ADD: {
    struct object o =
        concat_string_objects(left->value.string, right->value.string);
    vm_stack_cur(vm) = o;
    gc_add(vm, o);
  } break;
...
static void vm_do_binary_boolean_operation(
    const struct vm *restrict vm, const enum opcode opcode,
    struct object *restrict left, const struct object *restrict right) {
  switch (opcode) {
  case OPCODE_AND:
...
static void vm_do_binary_operation(struct vm *restrict vm,
  ...
  case OBJ_BOOL:
    ...
    break;
  default:
    err(VM_ERR_INVALID_OP_TYPE, "Invalid type %s for binary operation.",
        object_type_to_str(left->type));
    break;
  }
}

static void vm_do_integer_comparison(const struct vm *restrict vm,
                                     const enum opcode opcode,
                                     struct object *restrict left,
                                     const struct object *restrict right) {
  switch (opcode) {
...
static void vm_do_bool_comparison(const struct vm *restrict vm,
                                  const enum opcode opcode,
                                  struct object *restrict left,
                                  const struct object *restrict right) {
  switch (opcode) {
...
static void vm_do_string_comparison(const struct vm *restrict vm,
                                    const enum opcode opcode,
                                    struct object *restrict left,
                                    const struct object *restrict right) {
  left->type = OBJ_BOOL;
  ...
  case OPCODE_EQUAL:
    left->value.boolean =
        strcmp(left->value.string->value, right->value.string->value) == 0;
    break;
  ...
  default:
    err(VM_ERR_INVALID_OP_TYPE, "Invalid operator %s for string comparison.",
        opcode_to_str(opcode));
    break;
  }
}

static void vm_do_comparision(struct vm *restrict vm,
                              const enum opcode opcode) {
  const struct object *right = &vm_stack_pop(vm);
  struct object *left = &vm_stack_cur(vm);
  assert(left->type == right->type);
...
static void vm_do_bang_operation(struct vm *restrict vm) {
  // modify item in place by leaving it on the stack
  struct object *obj = &vm_stack_cur(vm);
  obj->value.boolean = obj->type == OBJ_NULL ||
                       (obj->type == OBJ_BOOL && obj->value.boolean == false) ||
                       (obj->type == OBJ_INT && obj->value.integer <= 0);
  obj->type = OBJ_BOOL;
}

static void vm_do_minus_operation(struct vm *restrict vm) {
  // modify item in place by leaving it on the stack
...
static void
vm_do_call_builtin(struct vm *restrict vm,
                   struct object (*builtin)(const struct object_list *),
...
static void vm_do_call_function(struct vm *restrict vm,
                                struct compiled_function *restrict fn,
                                uint8_t num_args) {
...
static void vm_do_call(struct vm *restrict vm, uint8_t num_args) {
...
static struct object vm_build_array(struct vm *restrict vm,
...
static struct object build_slice_from_array(struct object_list *source,
...
static struct object build_slice_from_string(struct string *source,
                                             int32_t start, int32_t end) {
  if (start < 0) {
...
static struct object build_slice(struct object left, struct object obj_start,
                                 struct object obj_end) {
  if ((obj_end.type != OBJ_NULL && obj_end.type != OBJ_INT) ||
      (obj_start.type != OBJ_NULL && obj_start.type != OBJ_INT)) {
...
enum result vm_run(struct vm *restrict vm) {
  /*
  The following comment is taken from CPython's source:
 https://github.com/python/cpython/blob/master/Python/ceval.c#L775
...
GOTO_OPCODE_JUMP_NOT_TRUE : {
  ...
  if (condition.type == OBJ_NULL ||
      (condition.type == OBJ_BOOL && condition.value.boolean == false)) {
    uint16_t pos = read_uint16((frame->ip + 1));
    frame->ip = frame->fn->instructions.bytes + pos;
  } else {
    frame->ip += 3;
...
GOTO_OPCODE_ARRAY : {
  ...
  frame->ip += 3;
  struct object array =
      vm_build_array(vm, vm->stack_pointer - num_elements, vm->stack_pointer);
...
GOTO_OPCODE_INDEX_GET : {
  ...
  if (index.type != OBJ_INT) {
    struct object obj =
        make_error_object("Array index must be integer or slice");
    vm_stack_push(vm, obj);
    gc_add(vm, obj);
    DISPATCH();
  }
  ...
  default: {
    struct object obj =
        make_error_object("Invalid left-hand side for indexing operation");
    vm_stack_push(vm, obj);
    gc_add(vm, obj);
  } break;
  }
...
GOTO_OPCODE_INDEX_SET : {
  ...
  if (index.value.integer < 0 || index.value.integer >= list->size) {
    vm_stack_push(vm,
                  make_error_object("Array assignment index out of bounds"));
    gc_add(vm, vm_stack_cur(vm));
  } else {
